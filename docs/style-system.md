...
- `ornament`: `ornament` annotation blocks must use configured entries from the `ornaments` theory catalog; `trill` blocks must emit a rapid alternation between two pitch classes.
